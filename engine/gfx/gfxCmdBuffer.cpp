...
namespace Mango
{
   ...
   void GFXCmdBuffer::bindTexture(uint32_t slot, TextureHandle texture)
   {
      int type = (int)CommandType::BindTexture;
      cmdBuffer[offset++] = type;

      cmdBuffer[offset++] = slot;
      cmdBuffer[offset++] = texture;
   }

   void GFXCmdBuffer::bindTextures(uint32_t startSlot, uint32_t count, TextureHandle* textures)
   {
      int type = (int)CommandType::BindTextures;
      cmdBuffer[offset++] = type;

      cmdBuffer[offset++] = startSlot;
      cmdBuffer[offset++] = count;
      for (uint32_t i = 0; i < count; ++i)
      {
